...
# model architecture (same as in the training script)
...
def evaluate(siamese_model, dataloader, triplet_criterion, device):
    ...
    # fraction of triplets where positive distance is smaller than negative
    correct_order = np.sum(all_distances_pos < all_distances_neg) / len(all_distances_pos)
    
    # fraction of triplets with zero loss
