...
def evaluate(siamese_model, classifier_model, dataloader, triplet_criterion, bce_criterion, device):
    ...
    with torch.no_grad():
        for i, (anchor, positive, negative) in enumerate(dataloader, start=1):
            ...
            # Forward pass through the classifier network for both positive and negative pairs
            positive_classifier_out = classifier_model(anchor_out, positive_out, negative_out)
            negative_classifier_out = classifier_model(anchor_out, negative_out, positive_out)
            ...
            all_labels.extend(np.ones_like(predictions_positive))  # Label 1 for positive pairs

            all_predictions.extend(predictions_negative)
            all_labels.extend(np.zeros_like(predictions_negative))  # Label 0 for negative pairs
            
            # Print predictions for debugging
            print(f'Batch {i}/{total_batches}')
            print(f'Positive Predictions: {predictions_positive[:5]}')  # Print first 5 predictions for positive samples
            print(f'Negative Predictions: {predictions_negative[:5]}')  # Print first 5 predictions for negative samples
