...
def evaluate(siamese_model, dataloader, criterion, device, threshold):
    ...
    with torch.no_grad():
        for anchor, positive, negative in pbar:
            ...
            pos_pred = (dist_pos < threshold).cpu().numpy()
            neg_pred = (dist_neg >= threshold).cpu().numpy()
            all_predictions.extend(pos_pred)
            all_predictions.extend(neg_pred)
            all_labels.extend(np.ones_like(pos_pred))
            all_labels.extend(np.zeros_like(neg_pred))
    ...
    return avg_loss, mean_pos_dist, mean_neg_dist, std_pos_dist, std_neg_dist, overlap_percentage, auc, accuracy, f1, mcc
...
# Evaluate
val_loss, mean_pos_dist, mean_neg_dist, std_pos_dist, std_neg_dist, overlap_percentage, auc, accuracy, f1, mcc = evaluate(siamese_net, val_dataloader, criterion, device, threshold)
