...
import subprocess
import collections
from readability2.langdata import LANGDATA
if sys.version[0] >= '3':
...
def getmeasures(text, lang='en', merge=False):
	...
	readability2 = collections.OrderedDict([
	...
	if basicwords:
		...
		readability2['DaleChallIndex'] = DaleChallIndex(
				words, complex_words_dc, sentences)
	if merge:
		readability2.update(stats)
		readability2.update(wordusage)
		readability2.update(beginnings)
		return readability
	return collections.OrderedDict([
			('readability grades', readability2),
			('sentence info', stats),
